...
class StarCraft2Env(MultiAgentEnv):
    ...
    def get_agent_action(self, a_id, action):
        ...
        if avail_actions[action] != 1:
            self.assert_count += 1
            print(f"当前动作无法执行，无法执行动作次数为 {self.assert_count} 次")
            random_list = [random.randint(0, 10) for _ in range(10)]
            i = 0
            while avail_actions[random_list[i]] != 1:
                i += 1
            action = random_list[i]
    ...
    def get_obs_agent(self, agent_id, fully_observable=False):
        ...
        if (
            ...
            #根据相对距离的大小从小往大排列obs
            enemy_feats = enemy_feats[enemy_feats[:, 1].argsort()]
            ally_feats = ally_feats[ally_feats[:, 1].argsort()]

            # Own features
            #判断是否进行random_mask,mask的概率为self.n_agents * 2:

            random_integer = random.randint(1, self.n_agents * 2)
            if random_integer != 1:
                ind = 0
                ...
                if self.unit_type_bits > 0:
                    type_id = self.get_unit_type_id(unit, True)
                    own_feats[ind + type_id] = 0

                #随机一个数进行mask，ally_feats的行较小，所以选用all_feats的大小
                random_integer = random.randint(0, ally_feats.shape[0]-1)
                enemy_feats[random_integer, :] = 0
                ally_feats[random_integer, :] = 0
